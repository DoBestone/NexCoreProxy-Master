    ...
    <div class="register-wrapper">
      <div class="register-card">
        ...
        <div class="register-header">
          ...
          <p class="register-subtitle">创建账号开始使用代理服务</p>
        </div>
        
        <!-- 注册表单 -->
        <a-form :model="form" @finish="handleRegister" layout="vertical" class="register-form">
          <a-form-item name="username" :rules="[{ required: true, message: '请输入用户名' }, { min: 3, message: '用户名至少3个字符' }]">
            <a-input v-model:value="form.username" placeholder="请输入用户名" size="large">
              <template #prefix><UserOutlined class="input-icon" /></template>
            </a-input>
          </a-form-item>
          
          <a-form-item name="email" :rules="[{ type: 'email', message: '请输入正确的邮箱' }]">
            <a-input v-model:value="form.email" placeholder="请输入邮箱（可选）" size="large">
              <template #prefix><MailOutlined class="input-icon" /></template>
            </a-input>
          </a-form-item>
          
          <a-form-item name="password" :rules="[{ required: true, message: '请输入密码' }, { min: 6, message: '密码至少6个字符' }]">
          ...
          <a-form-item name="inviteCode">
            <a-input v-model:value="form.inviteCode" placeholder="邀请码（可选）" size="large">
...
onUnmounted(() => {
  if (window.turnstile && turnstileWidgetId.value) window.turnstile.remove(turnstileWidgetId.value)
})
</script>
...
@media (max-width: 576px) { .register-card { padding: 28px 20px; margin: 16px; border-radius: 16px; } }
